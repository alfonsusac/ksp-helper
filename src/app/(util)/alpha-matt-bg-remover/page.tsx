...
export type FileData = { file: File, url: string }
...
export default function AlphaMatteBGRemover() {
  ...
  const [ matteColors, setMatteColors ] = useState<[ string, string ]>([ '#000', '#fff' ])
  ...
  useEffect(() => {
    const onPaste = async (e: ClipboardEvent) => {
      ...
      for (const item of items) {
        ...
        if (selected === 0) {
          setBlackImage({ file, url })
          setSelected(1)
          if (whiteImage) convert(matteColors, file, whiteImage.file)
        } else {
          setWhiteImage({ file, url })
          if (blackImage) convert(matteColors, blackImage.file, file)
        }
        break
      }
    }
  ...
  }, [ blackImage, whiteImage, selected ])
  ...
  // thanks chatgpt
  const convert = async (matteColors: [ string, string ], file1: File | undefined, file2: File | undefined) => {
    ...
    <div className={cns.page("")}>
              ...
              {image ? <>
                <input
                  ...
                  onChange={(e) => {
                    ...
                    matteColors[ i ] = val
                    convert(matteColors, blackImage?.file, whiteImage?.file)
      ...
      <div className="grid grid-cols-3 gap-4">
        <button className={cns.button.base("")} onClick={async () => {
          await convert(matteColors, blackImage?.file, whiteImage?.file)
        }}>Convert</button>
      </div>
